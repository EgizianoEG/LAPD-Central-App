...
This is a feature-rich Discord bot built with discord.js v14 for ER:LC LAPD roleplay communities. The bot provides utility commands, management modules, and roleplay-specific features. It's a TypeScript/Node.js application that uses MongoDB for data persistence and integrates with Roblox and Google APIs.
...
2. **Install dependencies** - Use `npm ci` for clean installs:
   ...
   npm ci
   ```
   Note: You may see engine warnings about Node.js version requirements (24.1.0) when running on lower versions (e.g., 20.x). The project still builds successfully despite these warnings.
...
- Some files have cognitive complexity warnings (expected, not blocking)
- `Source/Commands/Utility/ServerData/Subcmds/Manage.ts` has formatting issues
- A regex complexity warning in `Source/Resources/RegularExpressions.ts`
- These are pre-existing issues; don't worry about them unless you're modifying those files
...
**Known test issues:**
- 3 test suites fail due to a regex syntax error in `Source/Resources/RegularExpressions.ts` (duplicate capture group name)
- This is a pre-existing issue with the regex on line 46 that uses duplicate named groups `(?<username>)` and `(?<hr_time>)`
- Tests in `Tests/Utils/StringUtils.test.ts`, `Tests/Utils/Validator.test.ts`, and `Tests/Other/Resources.test.ts` fail due to this
- 2 test suites pass successfully (43 tests total pass in working suites)
...
│   ├── AppEmojis/            # Emoji definitions
│   ├── ERLC-Data/            # ER:LC game data
...
│   └── RegularExpressions.ts # Regex patterns (HAS KNOWN ISSUE)
...
└── Utilities/                 # Helper utilities (104 files, largest section)
    ├── Autocompletion/       # Command autocomplete handlers
    ├── Classes/              # Utility classes (e.g., AppLogger)
    ├── Database/             # Database utilities
    ├── Discord/              # Discord.js helpers
...
├── Utils/                    # Utility function tests (some fail due to regex issue)
└── Other/                    # Other tests (Resources.test.ts fails)
```
...
2. Setup Node.js v23.x (note: different from package.json requirement of 24.1.0)
3. **Run `npm run cp:secrets`** (creates dummy secrets file)
...
**Important:** The workflow uses Node.js 23.x, but package.json specifies 24.1.0. This works fine despite engine warnings.
...
- `*.log`, `logs/`
- `*.env` files
- PM2 config files (except examples)
...
- Database functions in `Source/Models/Functions/`
...
- Changes to these require a rebuild to take effect
...
- **express** v5.1.0 - HTTP server (for health checks)
- **axios** v1.11.0 - HTTP client
- **chalk** v5.5.0 - Terminal colors
- **winston** v3.11.0 - Logging
- **@napi-rs/canvas** v0.1.76 - Image generation
...
2. **The regex issue is pre-existing** - Don't try to fix `Source/Resources/RegularExpressions.ts` unless specifically tasked. The duplicate capture group issue affects some tests but doesn't block builds or the CI workflow.

3. **Path aliases are replaced during build** - The tsconfig-replace-paths step converts all `@Source`, `@Utilities` style imports to relative paths in the compiled output. This is why you see "Replaced 805 paths in 194 files" during builds.

4. **Resource files must be copied** - If you modify files in `Source/Resources/{Imgs,Fonts,HTML,Libs}`, you must rebuild to see changes in the Build/ directory.

5. **Node version flexibility** - Despite package.json requiring Node 24.1.0, the project builds successfully on Node 20.x and the CI uses Node 23.x. Engine warnings are expected and safe to ignore.

6. **Test failures are known** - Don't be alarmed by the 3 failing test suites. They fail in main branch too. Focus on not breaking the 2 passing test suites.

7. **CRLF line endings** - This project uses Windows-style (CRLF) line endings. Configure your editor accordingly or use the .editorconfig file.

8. **Pascal case convention** - Unlike typical JavaScript/TypeScript projects that use camelCase, this project uses PascalCase for most identifiers. Follow existing patterns in the file you're editing.
